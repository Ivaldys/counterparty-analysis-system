...
class MainScreen(Screen):
    ...
    COL_WIDTH_BASE = 180
    ROW_HEIGHT_BASE = 52
    HEADER_HEIGHT_BASE = 42
    TABLE_SPACING_BASE = 6
    ...
    def __init__(self, **kwargs):
        ...
        self.COL_WIDTH = dp(self.COL_WIDTH_BASE)
        self.ROW_HEIGHT = dp(self.ROW_HEIGHT_BASE)
        self.HEADER_HEIGHT = dp(self.HEADER_HEIGHT_BASE)
        self.TABLE_SPACING = dp(self.TABLE_SPACING_BASE)
    ...
    def use_server(self):
        ...
        try:
            ...
            onetime = onetime.rename(columns={
                "Аналитика Кт_0": "Название компании",
            ...
            final_df = final_df[
                final_df["ИНН"].notna() &
                final_df["ИНН"].astype(str).str.strip().ne("") &
                final_df["ИНН"].astype(str).str.fullmatch(r"\d+")
            ]

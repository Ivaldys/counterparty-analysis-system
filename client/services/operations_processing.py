...
def _detect_counterparty_column(df_filtered: pd.DataFrame) -> str:
    ...
    for col in kt_cols:
        series = df_filtered[col].fillna("").astype(str).str.strip()
        clean = series[~series.str.lower().isin({v.lower() for v in bad_values})]
        ...
        doc_like_count = clean.str.contains(
            r"(ПОСТУПЛЕНИЕ|СПИСАНИЕ|АВАНСОВЫЙ ОТЧЕТ|КОРРЕКТИРОВКА|ОТ \d{2}\.\d{2}\.\d{4})",
            case=False,
            regex=True,
            na=False,
        ).sum()

        contract_like_count = clean.str.contains(
            r"(ДОГОВОР|ОСНОВНОЙ ДОГОВОР|ЗАКАЗ)",
            case=False,
            regex=True,
            na=False,
        ).sum()

        score = marker_count * 100 - doc_like_count * 50 - contract_like_count * 20
